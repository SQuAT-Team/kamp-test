...
public class LameUtil {
    ...
    public static void initLame() throws ConversionException {
        ...
        File lame = new File("lame.exe");
        if (!lame.exists() || lame.length() == 0) {
            // get the input stream to lame.exe (in the classpath directory)
            InputStream is = new BufferedInputStream(LameUtil.class.getClassLoader().getResourceAsStream("lame.exe"));
            if (is != null) {
...
// should be written
     ...
     */
    public static boolean encode(String input, String output, int bitrate) {
        System.out.println("Encoding " + new File(input).getAbsolutePath() + " to "
                + new File(output).getAbsolutePath());
        String options = "-h --cbr -b " + bitrate + " \"" + input + "\" \"" + output + "\"";
        execLame(options);
     ...
     */
    public static boolean decode(String input, String output) {
        System.out.println("Decoding " + new File(input).getAbsolutePath() + " to "
                + new File(output).getAbsolutePath());
        String options = "--decode \"" + input + "\" \"" + output + "\"";
        execLame(options);
        return new File(output).exists() && new File(output).length() > 0;
    }
     ...
     */
    public static void execLame(String options) {
        Process p;
        try {
            // Launch lame encoder
            String lame = "lame";
            ...
            InputStreamReader pIsrErr = new InputStreamReader(p.getErrorStream());
            InputStreamReader pIsrIn = new InputStreamReader(p.getInputStream());
