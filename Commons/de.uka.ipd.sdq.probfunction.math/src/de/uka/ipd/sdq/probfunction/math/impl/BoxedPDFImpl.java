...
public class BoxedPDFImpl extends ProbabilityDensityFunctionImpl implements IBoxedPDF {
    ...
    public void checkConstrains() throws InvalidSampleValueException, UnitNameNotSetException, UnitNotSetException,
    ...
    public IProbabilityDensityFunction getCumulativeFunction() {
        List<Double> cumulativeProbabilities = MathTools.computeCumulativeProbabilities(getProbabilities());
    ...
    public IProbabilityDensityFunction stretchDomain(double scalar) {
        ...
        if (scalar != 0) {
            for (IContinuousSample oldSample : samples) {
                newSamples.add(pfFactory.createContinuousSample(oldSample.getValue() * scalar,
                        oldSample.getProbability()));
            }
        } else {
            // TODO: Is there a better way to handle a factor 0 for stretching the domain? Maybe
            // creating a static 0-PDF?
            // TODO: Introduce proper error handling in whole probfunction package.
            throw new RuntimeException("Error: Stretching the domain of PDF " + this.toString()
                    + " with factor 0 is undefined. Please change your models so that no PDf is multiplied by 0.");
        }
