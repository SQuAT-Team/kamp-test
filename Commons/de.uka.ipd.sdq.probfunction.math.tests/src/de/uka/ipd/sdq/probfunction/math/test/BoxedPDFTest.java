...
public class BoxedPDFTest {
    ...
    private IBoxedPDF createBoxedPDF(Double[] samples) throws DoubleSampleException {
        ...
        for (int i = 0; i < samples.length; i += 2) {
            IContinuousSample s = dfFactory.createContinuousSample(samples[i], samples[i + 1]);
            sList.add(s);
        }
    ...
    public void setUp() throws DoubleSampleException {
        df1 = createBoxedPDF(new Double[] {
                3.0, 0.1, 2.1, 0.2, 4.3, 0.4, 1.5, 0.3
        });
        df2 = createBoxedPDF(new Double[] {
                3.0, 0.0, 2.1, 0.2, 2.2, 0.1, 2.5, 0.2, 4.3, 0.3
        });
    ...
    public void equals() throws DoubleSampleException {
        ...
        IBoxedPDF df1copy = createBoxedPDF(new Double[] {
                3.0, 0.1, 2.1, 0.2, 4.3, 0.4, 1.5, 0.3
        });
        Assert.assertTrue(df1.equals(df1copy));

        Assert.assertFalse(df2.equals(df1));
        IBoxedPDF df1WrongCopy = createBoxedPDF(new Double[] {
                2.1, 0.2, 3.0, 0.1, 4.3, 0.4, 1.5, 0.3
        });
    ...
    public void scale() throws DoubleSampleException {
        ...
        IBoxedPDF spdf = createBoxedPDF(new Double[] {
                3.0, 0.01, 2.1, 0.02, 4.3, 0.04, 1.5, 0.03
        });
        Assert.assertEquals(spdf, df1scale);
    }

    @Test
    public void add() throws DoubleSampleException, FunctionsInDifferenDomainsException, UnknownPDFTypeException,
            IncompatibleUnitsException {
        IProbabilityDensityFunction sum = df1.add(df1);
        IBoxedPDF sumExpected = createBoxedPDF(new Double[] {
                3.0, 0.2, 2.1, 0.4, 4.3, 0.8, 1.5, 0.6
        });
        Assert.assertEquals(dfFactory.transformToSamplePDF(sumExpected), (ISamplePDF) sum);
    ...
    public void mult() throws FunctionsInDifferenDomainsException, UnknownPDFTypeException, IncompatibleUnitsException,
        ...
        IBoxedPDF expected = createBoxedPDF(new Double[] {
                3.0, 0.01, 2.1, 0.04, 4.3, 0.16, 1.5, 0.09
        });
    ...
    public void getMedian() throws UnorderedDomainException, DoubleSampleException {
        ...
        IBoxedPDF df = createBoxedPDF(new Double[] {
                3.0, 0.1, 2.1, 0.2, 4.3, 0.4, 1.5, 0.1, 1.4, 0.2
        });
    ...
    public void checkConstraints1() throws DoubleSampleException, NegativeDistanceException,
            ProbabilitySumNotOneException, FunctionNotInTimeDomainException, UnitNotSetException,
            UnitNameNotSetException, InvalidSampleValueException {
        IBoxedPDF b = createBoxedPDF(new Double[] {
                3.0, 0.1, 2.1, 0.4, 4.3, 0.3, -1.5, 0.2
        });
        b.checkConstrains();
    }

    @Test(expected = ProbabilitySumNotOneException.class)
    public void checkConstraints2() throws DoubleSampleException, NegativeDistanceException,
            ProbabilitySumNotOneException, FunctionNotInTimeDomainException, UnitNotSetException,
            UnitNameNotSetException, InvalidSampleValueException {
        IBoxedPDF b = createBoxedPDF(new Double[] {
                3.0, 0.1, 2.1, 0.4, 4.3, 0.3, 1.5, 0.3
        });
        b.checkConstrains();
    }
