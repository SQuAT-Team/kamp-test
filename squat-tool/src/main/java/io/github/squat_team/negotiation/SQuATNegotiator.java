...
public class SQuATNegotiator {
	...
	public PCMArchitectureInstance negotiate() throws Exception {
		...
		// Optional: show or output satisfaction of scenarios with current/initial
		// architecture
		// Red (bad) is for results under the threshold for maximization and over the
		// threshold for minimization
		// Yellow is for results near the threshold (useful for negotiation)
		// Green (good) is for results over the threshold for maximization and under the
		// threshold for minimization
		...
		for (PCMScenarioResult result : currentResults) {
			...
			if (result.isSatisfied() < 0) {
				// A bot searches for modifications which will (possible) lead to a better
				// satisfaction of its quality scenario
				...
				// In this point, we need to sort the alternatives of each bot either by their
				// response or by using a utility function
		...
		for (AbstractPCMBot bot : alternativesPerBot.keySet()) {
			...
			if (alternatives.size() > 0)
				...
				continue;
			// Try to apply the tactic first, else use a diff between the architectures to
			...
			if (tactic != null)
				...
				nextAchitecture = ArchitectureTransformer.transform(currentArchitecture, initialArchitecture,
						changedArchitecture);
