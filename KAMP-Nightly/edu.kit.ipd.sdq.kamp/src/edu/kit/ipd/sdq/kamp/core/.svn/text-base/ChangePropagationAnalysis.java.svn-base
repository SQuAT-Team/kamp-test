...
public class ChangePropagationAnalysis {
	...
	private void calculateAndMarkDataTypeToInterfacePropagation(ArchitectureVersion version) {
		...
		for (DataType datatype : initialMarkedDatatypes) {
			Map<OperationInterface, List<OperationSignature>> operationInterfaces = 
					ArchitectureModelLookup.lookUpInterfacesAndSignaturesWithParameterOfType(version, datatype);
			
			Map<OperationInterface, List<OperationSignature>> interfacesToBeMarked = new HashMap<>();
			
			for (Map.Entry<OperationInterface, List<OperationSignature>> operationInterfaceEntry : operationInterfaces.entrySet()) {
				if (!initialMarkedInterfaces.contains(operationInterfaceEntry.getKey())) {
					interfacesToBeMarked.put(operationInterfaceEntry.getKey(), operationInterfaceEntry.getValue());
				}
			}
			...
			if (!interfacesToBeMarked.isEmpty()) {
				...
				for (Map.Entry<OperationInterface, List<OperationSignature>> operationInterfaceEntry : interfacesToBeMarked.entrySet()) {
					...
					modifyInterface.setOperationInterface(operationInterfaceEntry.getKey());
					
					for (OperationSignature operationSignature: operationInterfaceEntry.getValue()) {
						ModifySignature modifySignature = modificationmarksFactory.eINSTANCE.createModifySignature();
						modifySignature.setToolderived(true);
						modifySignature.setSignature(operationSignature);
	...
	public List<RequiredRole> calculateInterComponentPropagation(List<ProvidedRole> newProvidedRolesMarked, ArchitectureVersion version) {
		...
		for (ProvidedRole providedRole : newProvidedRolesMarked) {
			...
			// required roles nur zur�ckgeben, wenn noch nicht markiert
