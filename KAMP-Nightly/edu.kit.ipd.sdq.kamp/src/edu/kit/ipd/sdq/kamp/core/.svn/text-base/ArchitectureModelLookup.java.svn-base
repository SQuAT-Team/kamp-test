...
public class ArchitectureModelLookup {
	...
	public static Map<OperationInterface, List<OperationSignature>> lookUpInterfacesAndSignaturesWithParameterOfType(
			ArchitectureVersion version, DataType datatype) {
		
		Map<OperationInterface, List<OperationSignature>> results = new HashMap<>();
		
		for (Interface interfac : version.getRepository().getInterfaces__Repository()) {
			collectInterfacesAndSignaturesWithDatatype(datatype, results, interfac);
		}
		
		return results;
	}
	
	private static void collectInterfacesAndSignaturesWithDatatype(DataType datatype,
			Map<OperationInterface, List<OperationSignature>> results, Interface interfac) {
	...
	private static void checkSignatureForDatatype(DataType datatype,
			Map<OperationInterface, List<OperationSignature>> results,
		...
		for (Parameter parameter : signature.getParameters__OperationSignature()) {
			if (parameter.getDataType__Parameter()==datatype) {
				if (!results.containsKey(operationInterface)) {
					results.put(operationInterface, new LinkedList<OperationSignature>());
				}
				if (!results.get(operationInterface).contains(signature)) {
					results.get(operationInterface).add(signature);
				}
			}
		}
